'use client'
import { LeaderboardTable, MultiplayerLeaderboardTable } from '@/features/leaderboard-table'
import {
  LeaderboardFilter,
    ...
    <div className='flex flex-col py-6'>
      <Tabs defaultValue="solo" className="w-full">
        <TabsList className="mb-6 max-sm:h-auto max-sm:w-full bg-[#393939] p-1 rounded-2xl">
          <TabsTrigger value="solo" className="px-6 py-2 rounded-xl font-bold max-sm:w-full data-[state=active]:bg-[#F5D91F] data-[state=active]:text-black text-white hover:text-white/80 transition-colors">
            Одиночная игра
          </TabsTrigger>
          <TabsTrigger value="multiplayer" className="px-6 py-2 rounded-xl font-bold max-sm:w-full data-[state=active]:bg-[#F5D91F] data-[state=active]:text-black text-white hover:text-white/80 transition-colors">
            Два игрока
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="solo" className="mt-0">
          ...
          {MODES.filter((mode) => filter === 'all' || filter === mode.key).map(
            (mode) => {
              const modeData = soloData.find((d) => d.timeMode === mode.timeMode)
